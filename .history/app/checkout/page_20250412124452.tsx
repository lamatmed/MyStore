...
export default function CheckoutPage() {
  ...
  const generateInvoice = () => {
    if (typeof window !== "undefined") {
      // Charger dynamiquement pdfmake et vfs_fonts
      import("pdfmake/build/pdfmake").then((pdfMakeModule) => {
        import("pdfmake/build/vfs_fonts").then((pdfFontsModule) => {
          const pdfMake: any = pdfMakeModule.default;  // Force type 'any'
          const pdfFonts: any = pdfFontsModule.default; // Force type 'any'

          // Assurez-vous que vfs est bien assigné avant de continuer
          if (pdfMake && pdfFonts) {
            pdfMake.vfs = pdfFonts.pdfMake.vfs;  // Assigner les polices
            ...
            console.error("Erreur : pdfMake ou vfs non chargé correctement");
          }
        }).catch((error) => {
          console.error("Erreur lors du chargement de vfs_fonts :", error);
        });
      }).catch((error) => {
